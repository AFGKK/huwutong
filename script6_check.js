    ...
    function toggleChat() {
        ...
        if (isOpen) {
            ...
            setTimeout(() => document.getElementById('chat-input');
        ...
        const welcome = '您好！我是' + sellerName + '的智能客服助手，很高兴为您服务！😊\n\n' +
        ...
        try {
            ...
            if (data.success && data.data) {
                ...
                if (reply) {
                    ...
                    appendChatMessage({ role: 'assistant', content: '感谢您的咨询，客服将尽快回复您！', timestamp: new Date().toISOString() });
            ...
            appendChatMessage({ role: 'assistant', content: '网络异常，请检查连接后重试', timestamp: new Date().toISOString() });
        ...
        try {
            ...
            if (data.success && data.data) {
                ...
                fetch('/api/chat/send', {
                    ...
                    body: JSON.stringify({ message: '用户发送了' + msg + '的' + fileUrl, session_id: _chatSessionId })
        ...
        try {
            ...
            if (data.success && data.data) {
                ...
                fetch('/api/chat/send', {
                    ...
                    body: JSON.stringify({ message: '[语音消息] ' + duration + '秒', session_id: _chatSessionId })
                ...
                appendChatMessage({ role: 'user', content: '语音发送失败' timestamp: new Date().toISOString() });
    ...
    function sendFaqMessage(msg) {
        ...
        .then(data => {
            ...
            if (data.success && data.data) {
                ...
                else appendChatMessage({ role: 'assistant', content: '感谢您的咨询！这个产品是 ' + name + '，如需了解更多请点击卡片查看详情', timestamp: new Date().toISOString() });
                ...
                appendChatMessage({ role: 'assistant', content: '感谢您的咨询！我已收到您发送的产品信息，客服将尽快为您服务', timestamp: new Date().toISOString() });
        ...
        .catch(() => {
            ...
            appendChatMessage({ role: 'assistant', content: '已收到您的产品卡片，客服将尽快回复您', timestamp: new Date().toISOString() });
    ...
    function requestHandoff() {
        ...
        if (!_token && !test) {
            ...
            if (ans) window.location.href = '/build/login';ld text-xs">test</div><div class="chat-msg-bubble bg-white dark-bg-card rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm border border-gray-100 dark-border"><div class="flex gap-1"><span class="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style="animation-delay:0s"></span><span class="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style="animation-delay:0.15s"></span><span class="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style="animation-delay:0.3s"></span></div></div></div>';
        ...
        fetch('/api/chat/send', {
            ...
            body: JSON.stringify({ message: '【转人工】用户请求转接人工客服，请帮助转接', session_id: _chatSessionId })
        ...
        .catch(() => {
            ...
            appendChatMessage({ role: 'assistant', content: '已收到您的转人工请求，客服人员将尽快接入，请耐心等待', timestamp: new Date().toISOString() });
