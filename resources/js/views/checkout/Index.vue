  ...
  <div class="checkout-page">
    <div v-if="loading" class="flex items-center justify-center min-h-[70vh]">
      <div class="text-center">
        ...
        <p class="text-gray-400 text-sm mt-4">正在加载...</p>
      </div>
    </div>
    <div v-else-if="error" class="flex items-center justify-center min-h-[70vh]">
      <el-result icon="error" title="加载失败" :sub-title="error">
        <template #extra>
          <el-button type="primary" round @click="$router.push('/products')">返回商城</el-button>
        </template>
      </el-result>
    </div>
    <div v-else>
      <div class="jd-topbar">
        <div class="jd-topbar-inner">
          <router-link to="/" class="jd-logo">
            <span class="jd-logo-icon">互</span>
            <span class="jd-logo-text">互物通</span>
          </router-link>
          <div class="jd-topbar-nav">
            <span class="jd-nav-item">首页</span>
            <span class="jd-nav-sep">&gt;</span>
            <span class="jd-nav-item">产品商城</span>
            <span class="jd-nav-sep">&gt;</span>
            <span class="jd-nav-item jd-nav-cur">确认订单</span>
          </div>
        </div>
      </div>
      <div class="jd-stepbar">
        <div class="jd-stepbar-inner">
          <div class="jd-step active"><span class="jd-step-num">1</span> 选择商品</div>
          <div class="jd-step active"><span class="jd-step-num">2</span> 确认订单</div>
          <div class="jd-step"><span class="jd-step-num">3</span> 完成支付</div>
        </div>
      </div>
      <div class="main-wrap">
        <div class="main-layout">
          <div class="main-left">
            <div class="section-h2">商品信息</div>
            <div class="goods-table">
              <div class="goods-thead">
                <span class="goods-col goods-col-info">商品</span>
                <span class="goods-col goods-col-price">单价</span>
                <span class="goods-col goods-col-qty">数量</span>
                <span class="goods-col goods-col-total">小计</span>
              </div>
              <div class="goods-trow">
                <div class="goods-col goods-col-info">
                  ...
                  <div class="goods-meta">
                    ...
                    <div class="goods-sku">{{ selectedPlan?.name || '标准方案' }}</div>
                  </div>
                </div>
            ...
            <div class="plan-tmall">
              <label v-for="p in plans" :key="p.id" class="plan-trow" :class="{ sel: selectedPlanId === p.id }">
                ...
                <span class="plan-tsub">{{ p.description || '标准授权' }}</span>
                <span class="plan-tprice">¥{{ p.price }}{{ periodLabelFor(p) }}</span>
              </label>
            </div>
            <div class="section-h2">数量与优惠</div>
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <div class="coupon-tmall">
                  <input v-model="couponCode" placeholder="输入优惠码" class="coupon-tinp" @keyup.enter="applyCoupon">
                  <button class="coupon-tbtn" :class="{ on: couponCode }" :disabled="!couponCode" @click="applyCoupon">验证</button>
                </div>
              </div>
            </div>
            <div class="section-h2">支付方式</div>
            <div class="pay-tmall">
              ...
              <label class="pay-topt" :class="{ sel: paymentMethod === 'wxpay' }">
                ...
                <span class="pay-tname">微信支付</span>
              </label>
              <label class="pay-topt" :class="{ sel: paymentMethod === 'balance' }">
                ...
                <span class="pay-tname">余额支付</span>
              </label>
            </div>
            <!-- ═══════ 自动续费 ═══════ -->
            <div v-if="isSubscription" class="section-h2">续费设置</div>
            <div v-if="isSubscription" class="renew-box">
              <label class="renew-toggle">
                ...
                <span class="renew-label">
                  ...
                  <span class="renew-desc">当前方案到期后将自动续费，可随时取消</span>
                </span>
              </label>
            </div>
            <!-- ═══════ 联系信息 ═══════ -->
            <div class="section-h2">联系信息</div>
            <div class="contact-box">
              <div class="contact-row">
                <div class="contact-field flex-1">
                  <label class="contact-label">收件邮箱 <span class="text-red-400">*</span></label>
                  <input v-model="contact.email" placeholder="用于接收 License 和订单通知" class="contact-input" type="email" />
                  <p class="contact-hint">授权码和订单凭证将发送到此邮箱</p>
                </div>
                <div class="contact-field flex-1">
                  <label class="contact-label">手机号码</label>
                  <input v-model="contact.phone" placeholder="选填，用于重要通知" class="contact-input" type="tel" maxlength="11" />
                  <p class="contact-hint">仅用于重要订单通知</p>
                </div>
              </div>
              <div class="contact-field">
                <label class="contact-label">收货人姓名</label>
                <input v-model="contact.name" placeholder="选填" class="contact-input" />
              </div>
            </div>
            <!-- ═══════ 订单备注 ═══════ -->
            <div class="section-h2">订单备注 <span class="section-sub">（选填）</span></div>
            <div class="notes-box">
              <textarea v-model="orderNotes" placeholder="如有特殊要求，请在此备注（如：发票邮寄地址、授权绑定域名等）" class="notes-textarea" rows="3" maxlength="500"></textarea>
              <div class="notes-counter">{{ orderNotes.length }}/500</div>
            </div>
            <!-- ═══════ 发票信息 ═══════ -->
            <div class="section-h2">发票信息 <span class="section-sub">（选填）</span></div>
            <div class="invoice-box">
              ...
              <div class="invoice-fields">
                <div class="invoice-field">
                  ...
                  <input v-model="invoice.title" placeholder="个人或企业名称" class="invoice-input" />
                </div>
                <template v-if="invoiceType === 'company'">
                  <div class="invoice-field">
                    <label class="invoice-label">统一社会信用代码</label>
                    <input v-model="invoice.tax_no" placeholder="18 位统一社会信用代码" class="invoice-input" maxlength="18" />
                  </div>
                  <div class="invoice-field">
                    <label class="invoice-label">公司地址</label>
                    <input v-model="invoice.address" placeholder="注册地址" class="invoice-input" />
                  </div>
                  <div class="invoice-field">
                    <label class="invoice-label">公司电话</label>
                    <input v-model="invoice.phone" placeholder="注册电话" class="invoice-input" />
                  </div>
                  <div class="invoice-row">
                    <div class="invoice-field flex-1">
                      <label class="invoice-label">开户银行</label>
                      <input v-model="invoice.bank" placeholder="如：中国工商银行" class="invoice-input" />
                    </div>
                    <div class="invoice-field flex-1">
                      <label class="invoice-label">银行账号</label>
                      <input v-model="invoice.bank_account" placeholder="银行账号" class="invoice-input" />
                    </div>
                  </div>
                </template>
              </div>
          ...
          <div class="main-right">
            <div class="side-card">
              ...
              <div class="side-total">
                <span class="side-total-label">应付总额：</span>
      ...
      <div class="checkout-footer">
        ...
        <div class="footer-inner">
          <div class="footer-grid">
            <div class="footer-brand">
              <a href="/" class="footer-logo">
                <div class="footer-logo-icon">互</div>
                <span class="footer-logo-text">{{ siteSettings.site_name || '互物通' }}</span>
              </a>
              <p class="footer-desc">{{ siteSettings.site_description || '企业级授权管理系统，为软件产品提供从激活验证到商业运营的一站式授权解决方案。' }}</p>
              <div class="footer-social">
                <a href="/contact" class="social-icon hover-green" title="微信">
                ...
                <a href="https://weibo.com/huwutong" target="_blank" rel="noopener noreferrer" class="social-icon hover-red" title="微博">
                ...
                <a href="https://github.com/huwutong" target="_blank" rel="noopener noreferrer" class="social-icon hover-gray" title="GitHub">
                ...
                <a href="mailto:support@huwutong.com" class="social-icon hover-blue" title="邮箱">
            ...
            <div class="footer-col">
              ...
              <a href="/#features" class="footer-link">产品特性</a>
              <a href="/products" class="footer-link">产品商城</a>
              <a href="/pricing" class="footer-link">定价</a>
              <a href="/compare" class="footer-link">竞品对比</a>
            </div>
            <div class="footer-col">
              <h4 class="footer-col-title">开发者</h4>
              <a href="/sdk" class="footer-link">SDK 下载</a>
              <a href="/blog" class="footer-link">开发者博客</a>
              <a href="/help" class="footer-link">帮助中心</a>
              <a href="/build/login" class="footer-link">管理后台</a>
            </div>
            <div class="footer-col">
              <h4 class="footer-col-title">公司</h4>
          ...
          <div class="footer-bottom">
            <p class="footer-copy">&copy; {{ new Date().getFullYear() }} {{ siteSettings.footer_copyright || '互物通 - 企业级授权管理系统' }}</p>
            <div v-if="siteSettings.icp_beian || siteSettings.gongan_beian" class="footer-beian-wrap">
              <a v-if="siteSettings.icp_beian" :href="siteSettings.icp_beian_url || '#'" target="_blank" rel="noopener noreferrer" class="beian-link">
                ...
                {{ siteSettings.icp_beian }}
              </a>
              <a v-if="siteSettings.gongan_beian" :href="siteSettings.gongan_beian_url || '#'" target="_blank" rel="noopener noreferrer" class="beian-link">
      ...
      <el-dialog v-model="showPayDialog" top="8vh" width="400px" class="pay-dialog" :close-on-click-modal="false" destroy-on-close>
        <template #header>
          <span class="text-base font-bold text-gray-800">确认支付</span>
        </template>
        <div class="text-center py-4">
          <div class="text-sm text-gray-400 mb-1">支付金额</div>
          <div class="text-3xl font-bold" style="color:#409eff">¥{{ total }}</div>
          <div class="text-xs text-gray-300 mt-3">支付方式：{{ paymentMethods.find(p => p.value === paymentMethod)?.label }}</div>
        </div>
        <template #footer>
          <div class="flex gap-2">
            <button class="dlg-btn2 dlg-cancel2" @click="showPayDialog = false">取消</button>
            <button class="dlg-btn2 dlg-confirm2" :disabled="paying" @click="handlePay">{{ paying ? '支付中...' : '确认支付' }}</button>
          </div>
        </template>
      </el-dialog>
    </div>
...
function periodLabelFor(plan) {
    ...
    const labels = { monthly: '/月', quarterly: '/季', yearly: '/年', 'one-time': '一次性' };
    return labels[cycle] || '/月';
}
...
onMounted(async () => {
  ...
  try {
        ...
        if (sku && sku.id) {
          ...
          if (currentProduct) {
            ...
            recommendProducts.value = others.slice(0, 3).map(p => ({
              id: p.id, name: p.name, price: parseFloat(p.lowest_price || p.base_price || 0) || '待定', added: false,
    ...
    if ((!plans.value || plans.value.length === 0) && product.value) { plans.value = [{ id: 0, product_id: product.value.id, name: '标准授权', description: '', price: 0, billing_cycle: 'monthly' }]; selectedPlanId.value = 0; }
...
async function applyCoupon() {
  ...
  if (!selectedPlan.value || !product.value) { ElMessage.warning('请先选择方案'); return; }
  ...
  try {
    ...
    if (d.success && d.data?.valid) {
      ...
      ElMessage.success('优惠码已应用，优惠 ¥' + (d.data.discount || 0).toFixed(2));
    } else {
      discount.value = 0;
      ElMessage.warning(d.data?.message || d.data?.error || '优惠码无效或已过期');
    }
  } catch (e) { ElMessage.error('验证优惠码失败'); discount.value = 0; }
}
async function submitOrder() {
  if (!selectedPlan.value) { ElMessage.warning('请选择定价方案'); return; }
  if (!contact.value.email) { ElMessage.warning('请填写收件邮箱'); return; }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact.value.email)) { ElMessage.warning('邮箱格式不正确'); return; }
...
.jd-logo-icon { width: 26px; height: 26px; background: linear-gradient(135deg, #409eff, #337ecc); border-radius: 6px; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 13px; font-weight: 700; box-shadow: 0 2px 6px rgba(64,158,255,0.3); }
...
.jd-step.active .jd-step-num { background: linear-gradient(135deg, #409eff, #337ecc); box-shadow: 0 2px 6px rgba(64,158,255,0.35); }
...
.plan-trow.sel { background: var(--primary-light); border-color: var(--primary); box-shadow: 0 0 0 3px rgba(64,158,255,0.08); }
...
.side-total-price { font-size: 24px; font-weight: 700; background: linear-gradient(135deg, #409eff, #2563eb); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
...
.order-btn { width: 100%; height: 44px; border: none; background: linear-gradient(135deg, #409eff, #2563eb); color: #fff; font-size: 15px; font-weight: 700; border-radius: var(--radius-sm); cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: var(--transition); box-shadow: 0 4px 12px rgba(64,158,255,0.3); }
.order-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(64,158,255,0.4); }
...
.invoice-input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(64,158,255,0.1); }
...
.renew-checkbox:checked + .renew-slider { background: linear-gradient(135deg, #409eff, #337ecc); }
...
.contact-input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(64,158,255,0.1); }
...
.notes-textarea:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(64,158,255,0.1); }
...
.footer-grid { display: grid; grid-template-columns: 1.8fr 1fr 1fr 1fr; gap: 24px; }
.footer-logo { display: flex; align-items: center; gap: 8px; text-decoration: none; margin-bottom: 12px; }
.footer-logo-icon { width: 34px; height: 34px; background: linear-gradient(135deg, #60a5fa, #2563eb); border-radius: 10px; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 16px; font-weight: 700; box-shadow: 0 4px 12px rgba(59,130,246,0.3); }
...
.social-icon.hover-red:hover { background: #ef4444; color: #fff; }
.social-icon.hover-gray:hover { background: #6b7280; color: #fff; }
.social-icon.hover-blue:hover { background: #3b82f6; color: #fff; }
.footer-col-title { font-size: 13px; font-weight: 600; color: #fff; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 14px; }
...
.footer-copy { font-size: 13px; color: #6b7280; margin-bottom: 8px; }
.footer-beian-wrap { display: flex; align-items: center; justify-content: center; gap: 24px; flex-wrap: wrap; }
.beian-link { display: inline-flex; align-items: center; gap: 6px; font-size: 12px; color: #9ca3af; text-decoration: none; transition: color 0.2s; }
.beian-link svg { width: 16px; height: 16px; color: #d1d5db; }
.beian-link:hover { color: #d1d5db; }
...
.dlg-confirm2 { background: linear-gradient(135deg, #409eff, #2563eb); color: #fff; border: none; font-weight: 600; box-shadow: 0 4px 12px rgba(64,158,255,0.3); }
.dlg-confirm2:hover:not(:disabled) { background: linear-gradient(135deg, #337ecc, #1d4ed8); transform: translateY(-1px); }
