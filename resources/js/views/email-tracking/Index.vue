    ...
    <div class="email-tracking-page">
        <div class="page-header">
            <div class="header-left">
                <h2>{{ t('email_tracking_page.title') }}</h2>
                <span class="header-subtitle">{{ t('email_tracking_page.subtitle') }}</span>
            </div>
        </div>
        ...
        <el-row :gutter="16" class="info-row">
                ...
                <el-card shadow="never">
                    <template #header>
                        <div class="card-header">
                            <span>{{ t('email_tracking_page.daily_trend') }}</span>
                        </div>
                    </template>
            ...
            <el-col :span="8">
                <el-card shadow="never">
                    <template #header>
                        <span>{{ t('email_tracking_page.bounce_reasons') }}</span>
                    </template>
                    <div v-loading="bounceLoading">
                        ...
                        <el-empty v-else :description="t('email_tracking_page.empty_bounce')" :image-size="60" />
                    </div>
                </el-card>
            </el-col>
        </el-row>
        ...
        <el-card shadow="never" class="table-card">
            <template #header>
                <span>{{ t('email_tracking_page.by_template') }}</span>
            </template>
            <el-table :data="byTemplate" v-loading="loading" stripe>
                <el-table-column :label="t('email_tracking_page.cols.template_code')" width="200" prop="template_code">
                ...
                <el-table-column :label="t('email_tracking_page.cols.sent')" width="80" prop="total_sent" align="center" />
                <el-table-column :label="t('email_tracking_page.cols.delivered')" width="80" prop="delivered" align="center" />
                <el-table-column :label="t('email_tracking_page.cols.delivery_rate')" width="90" align="center">
                    <template #default="{ row }">
                ...
                <el-table-column :label="t('email_tracking_page.cols.opened')" width="80" prop="opened" align="center" />
                <el-table-column :label="t('email_tracking_page.cols.open_rate')" width="90" align="center">
                    <template #default="{ row }">
                ...
                <el-table-column :label="t('email_tracking_page.cols.clicked')" width="80" prop="clicked" align="center" />
                <el-table-column :label="t('email_tracking_page.cols.click_rate')" width="90" align="center">
                    <template #default="{ row }">
                ...
                <el-table-column :label="t('email_tracking_page.cols.bounced')" width="80" prop="bounced" align="center" />
                <el-table-column :label="t('email_tracking_page.cols.failed')" width="80" prop="failed" align="center" />
                <el-table-column :label="t('email_tracking_page.cols.actions')" width="100">
                    <template #default="{ row }">
                        <el-button text size="small" type="primary" @click="drillDown(row)">{{ t('email_tracking_page.drill_down') }}</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-card>

        <el-dialog v-model="drillDownVisible" :title="t('email_tracking_page.drill_title', { code: drillDownCode })" width="900px" top="5vh">
            <div v-loading="drillDownLoading">
                ...
                <h4 class="section-title">{{ t('email_tracking_page.daily_trend_section') }}</h4>
                <div ref="drillChartRef" class="chart-container-sm"></div>

                <h4 class="section-title mt-4">{{ t('email_tracking_page.send_logs') }}</h4>
                <el-table :data="drillDownLogs" stripe size="small">
                    <el-table-column :label="t('email_tracking_page.cols.recipient')" width="180" prop="to_email" />
                    <el-table-column :label="t('email_tracking_page.cols.subject')" min-width="200" prop="subject" show-overflow-tooltip />
                    <el-table-column :label="t('email_tracking_page.cols.status')" width="100" prop="status">
                        <template #default="{ row }">
                    ...
                    <el-table-column :label="t('email_tracking_page.cols.sent_at')" width="170" prop="created_at">
                        <template #default="{ row }">{{ formatDate(row.created_at) }}</template>
                    </el-table-column>
                    <el-table-column :label="t('email_tracking_page.cols.opened_at')" width="170" prop="opened_at">
...
const funnelItems = computed(() => {
    ...
    return [
        { label: t('email_tracking_page.funnel.total_sent'), value: f.total_sent, rate: '100%', color: '#0f172a', percent: 100 },
        { label: t('email_tracking_page.funnel.delivered'), value: f.delivered, rate: f.delivery_rate + '%', color: '#67C23A', percent: maxVal > 0 ? (f.delivered / maxVal * 100) : 0 },
        { label: t('email_tracking_page.funnel.opened'), value: f.opened, rate: f.open_rate + '%', color: '#E6A23C', percent: maxVal > 0 ? (f.opened / maxVal * 100) : 0 },
        { label: t('email_tracking_page.funnel.clicked'), value: f.clicked, rate: f.click_rate + '%', color: '#F56C6C', percent: maxVal > 0 ? (f.clicked / maxVal * 100) : 0 },
    ]
})
...
function statusLabel(status) {
    const key = { delivered: 'delivered', opened: 'opened', clicked: 'clicked', bounced: 'bounced', failed: 'failed' }[status]
    return key ? t(`email_tracking_page.statuses.${key}`) : status
}
...
async function loadOverview() {
    ...
    try {
        ...
        ElMessage.error(t('email_tracking_page.messages.load_failed'))
    } finally {
        loading.value = false
    }
}
...
function renderChart() {
    ...
    const datasets = [
        { label: t('email_tracking_page.chart.sent'), key: 'total', color: '#0f172a' },
        { label: t('email_tracking_page.chart.delivered'), key: 'delivered', color: '#67C23A' },
        { label: t('email_tracking_page.chart.opened'), key: 'opened', color: '#E6A23C' },
    ]
    ...
    try {
        ...
        if (res.success) {
            ...
            drillDownFunnel.value = [
                { label: t('email_tracking_page.chart.sent'), value: f.total_sent, rate: '100%' },
                { label: t('email_tracking_page.chart.delivered'), value: f.delivered, rate: f.delivery_rate + '%' },
                { label: t('email_tracking_page.chart.opened'), value: f.opened, rate: f.open_rate + '%' },
                { label: t('email_tracking_page.funnel.clicked'), value: f.clicked, rate: f.click_rate + '%' },
            ]
        ...
        ElMessage.error(t('email_tracking_page.messages.template_failed'))
    } finally {
