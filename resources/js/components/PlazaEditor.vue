<template>
  <div class="plaza-editor">
    <div class="editor-toolbar" v-if="editor">
      <el-tooltip :content="t('plaza_editor.bold')" :show-after="300">
      ...
      <el-tooltip :content="t('plaza_editor.h2')" :show-after="300">
      ...
      <el-tooltip :content="t('plaza_editor.h3')" :show-after="300">
      ...
      <span class="toolbar-sep"></span>
      <el-tooltip :content="t('plaza_editor.bullet_list')" :show-after="300">
        <button @click="editor.chain().focus().toggleBulletList().run()" :class="{ 'is-active': editor.isActive('bulletList') }" class="editor-btn">•••</button>
      </el-tooltip>
      <el-tooltip :content="t('plaza_editor.ordered_list')" :show-after="300">
      ...
      <el-tooltip :content="t('plaza_editor.quote')" :show-after="300">
        <button @click="editor.chain().focus().toggleBlockquote().run()" :class="{ 'is-active': editor.isActive('blockquote') }" class="editor-btn">"</button>
      </el-tooltip>
      <span class="toolbar-sep"></span>
      <el-tooltip :content="t('plaza_editor.insert_link')" :show-after="300">
        <button @click="addLink" :class="{ 'is-active': editor.isActive('link') }" class="editor-btn">URL</button>
      </el-tooltip>
      <el-tooltip :content="t('plaza_editor.insert_image')" :show-after="300">
        <button @click="selectImage" class="editor-btn">Img</button>
      </el-tooltip>
      ...
      <el-tooltip :content="t('plaza_editor.code_block')" :show-after="300">
        <button @click="editor.chain().focus().toggleCodeBlock().run()" :class="{ 'is-active': editor.isActive('codeBlock') }" class="editor-btn">&lt;/&gt;</button>
      </el-tooltip>
      <span class="toolbar-sep"></span>
      <el-tooltip :content="t('plaza_editor.clear_format')" :show-after="300">
...
const editor = useEditor({
  ...
  extensions: [
    ...
    Placeholder.configure({
      placeholder: () => props.placeholder || t('plaza_editor.placeholder'),
    }),
  ],
  onUpdate: ({ editor }) => {
...
function addLink() {
  ...
  ElMessageBox.prompt(t('plaza_editor.link_prompt'), t('plaza_editor.link_title'), {
    inputValue: previousUrl || 'https://',
    inputPattern: /^https?:\/\/.+/,
    inputErrorMessage: t('plaza_editor.link_invalid'),
...
async function handleImageUpload(e) {
  ...
  try {
    ...
    if (res.success && res.data?.url) {
      ...
      ElMessage.error(res.message || t('plaza_editor.upload_fail'))
    }
  } catch {
    ElMessage.error(t('plaza_editor.image_upload_fail'))
  }
}
...
.editor-btn.is-active { background: #0f172a; color: #fff; }
...
.editor-content :deep(.ProseMirror blockquote) { border-left: 3px solid #0f172a; padding-left: 12px; color: #666; margin: 8px 0; }
...
.editor-content :deep(.ProseMirror a) { color: #0f172a; text-decoration: underline; }
