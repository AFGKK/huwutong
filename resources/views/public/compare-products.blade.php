...
<head>
    ...
    <title>产品对比 - 互物通| 企业级授权管理系统</title>
    <meta name="description" content="产品功能对比，帮助您选择最合适的方案">
    ...
    <section class="pt-24 pb-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-sm text-gray-500 flex items-center gap-2 mb-6">
                ...
                <span>/</span>
                <span class="text-gray-900 font-medium">产品对比</span>
            </div>
            <!-- 标题 -->
            <div class="text-center mb-12">
                <h1 class="text-3xl font-bold text-gray-900">产品对比</h1>
                <p class="text-gray-500 mt-2">比较不同产品的功能与定价，选择最适合您的方案</p>
            </div>
            ...
            @if($products->count() < 2)
                <div class="text-center py-20">
                    ...
                    <p class="text-lg text-gray-500 mb-2">请选择至少 2 个产品进行对比</p>
                    <p class="text-sm text-gray-400 mb-6">在产品详情页点击「加入对比」按钮添加产品</p>
                    <a href="/products" class="inline-block bg-primary-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-primary-700 transition">浏览产品</a>
                </div>
            @else
                <!-- 对比表格 -->
                <div class="overflow-x-auto bg-white rounded-2xl shadow-sm border border-gray-100">
                    <table class="compare-table w-full text-sm">
                        <thead>
                            <tr>
                                <th class="text-left w-40 bg-white sticky left-0 z-20" style="min-width:140px">对比项</th>
                                @foreach($products as $p)
                                    <th class="min-w-[200px]">
                                        <div class="flex flex-col items-center gap-2 py-2">
                                            ...
                                            <a href="{{ url('/products/'.$p->slug) }}" class="font-semibold text-gray-900 hover:text-primary-600">{{ $p->name }}</a>
                                        </div>
                                    </th>
                                @endforeach
                            </tr>
                        ...
                        <tbody>
                            <!-- 价格 -->
                            <tr>
                                <td class="text-left font-medium text-gray-700 bg-gray-50 sticky left-0">价格</td>
                                @foreach($products as $p)
                                    <td>
                            ...
                            <!-- 评分 -->
                            <tr>
                                <td class="text-left font-medium text-gray-700 bg-gray-50 sticky left-0">评分</td>
                                @foreach($products as $p)
                                    <td>
                                        ...
                                        <span class="text-gray-400 text-xs">({{ $p->review_count }}条评价)</span>
                                    </td>
                                @endforeach
                            </tr>
                            <!-- 已售 -->
                            <tr>
                                <td class="text-left font-medium text-gray-700 bg-gray-50 sticky left-0">已售</td>
                                @foreach($products as $p)
                            ...
                            <!-- 分类 -->
                            <tr>
                                <td class="text-left font-medium text-gray-700 bg-gray-50 sticky left-0">分类</td>
                                @foreach($products as $p)
                                    <td>
                                        @if($p->category)
                                            <span class="text-primary-600 bg-primary-50 px-3 py-1 rounded-full text-xs">{{ $p->category->name }}</span>
                                        @else
                            ...
                            <!-- 功能特性-->
                            <tr>
                                <td class="text-left font-medium text-gray-700 bg-gray-50 sticky left-0">功能特性</td>
                                @foreach($products as $p)
                                    <td>
                            ...
                            <!-- SKU 方案 -->
                            <tr>
                                <td class="text-left font-medium text-gray-700 bg-gray-50 sticky left-0">可选方案</td>
                                @foreach($products as $p)
                                    <td>
                                            ...
                                            <div class="space-y-1.5">
                                                @foreach($p->skus as $sku)
                                                    <div class="text-xs text-gray-600 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                                                        ...
                                                        @endif
                                                        <span class="text-gray-400 ml-2">/{{ $sku->billing_cycle === 'yearly' ? '年' : ($sku->billing_cycle === 'monthly' ? '月' : '') }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @else
                            ...
                            <tr>
                                <td class="text-left font-medium text-gray-700 bg-gray-50 sticky left-0" colspan="{{ $products->count() + 1 }}">
                                    <span class="text-base">规格参数</span>
                                </td>
                            </tr>
                            @php
                                $_allSpecs = [];
                ...
                <div class="flex justify-center gap-4 mt-10">
                    <a href="/products" class="inline-block bg-primary-600 text-white px-5 py-2 rounded-lg font-medium hover:bg-primary-700 transition">继续浏览</a>
                    <button onclick="clearCompare()" class="px-8 py-3 rounded-xl border border-red-200 text-red-500 hover:bg-red-50 transition font-medium text-sm">清空对比列表</button>
                </div>
            @endif
        </div>
    </section>
