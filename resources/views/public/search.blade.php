        ...
        try {
            ...
            if (j.success && j.data.suggestions.length > 0) {
                ...
                for (var si = 0; si < j.data.suggestions.length; si++) {
                    ...
                    else if (s.type === 'marketplace_apps') directLink = '/marketplace/' + (s.slug || s.id || '');
            ...
            fetch('/api/meilisearch/trending').then(function(tr) { return tr.json(); }).then(function(tj) {
                if (tj.success && tj.data.length > 0) {
                    ...
                    for (var ti = 0; ti < tj.data.length; ti++) {
                        ...
                        else if (t.type === 'marketplace_apps') tLink = '/marketplace/' + (t.slug || t.id || '');
...
function renderResults() {
    ...
    var links = {products:function(h){return '/products/'+(h.slug||h.id);},kb_articles:function(h){return '/help/'+h.id;},marketplace_apps:function(h){return '/marketplace/'+(h.slug||h.id);},forum_posts:function(h){return '/build/plaza/'+h.id;},blog_posts:function(h){return '/blog/'+(h.slug||h.id);},oa_articles:function(h){return '/build/oa-article/'+h.id;},users:function(){return '#';},official_accounts:function(h){return '/build/channels?account='+h.id;}};
