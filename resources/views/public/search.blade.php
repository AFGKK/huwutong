        ...
        <div class="w-full max-w-3xl mx-auto text-center">
            ...
            <form id="search-form" class="relative max-w-2xl mx-auto mb-2" onsubmit="return doSearch(event)">
                <div class="flex items-center bg-white border-2 border-gray-200 rounded-2xl shadow-sm hover:shadow-md hover:border-primary-300 focus-within:border-primary-500 focus-within:shadow-md transition-all duration-200">
                    ...
                    <input id="search-input" type="text" class="flex-1 border-0 bg-transparent px-3 py-4 text-lg text-gray-900 placeholder-gray-400 focus:ring-0 outline-none" placeholder="搜索商品、文章、应用、用户..." autocomplete="off" value="{{ request('q') }}" />
            ...
            <div class="flex items-center justify-center gap-2 flex-wrap text-xs mt-1">
                ...
                <a href="/search?type=products" class="text-gray-400 hover:text-primary-500 transition">📦 商品</a>
                <a href="/search?type=kb_articles" class="text-gray-400 hover:text-primary-500 transition">📖 帮助中心</a>
                <a href="/search?type=forum_posts" class="text-gray-400 hover:text-primary-500 transition">💬 社区</a>
                <a href="/search?type=blog_posts" class="text-gray-400 hover:text-primary-500 transition">📝 博客</a>
                <a href="/search?type=oa_articles" class="text-gray-400 hover:text-primary-500 transition">📢 互物号</a>
                <a href="/search?type=marketplace_apps" class="text-gray-400 hover:text-primary-500 transition">🧩 应用市场</a>
                <a href="/search?type=official_accounts" class="text-gray-400 hover:text-primary-500 transition">🏢 互物号账号</a>
                <span class="text-gray-300">∣</span>
            </div>
            <!-- 搜索历史 -->
            ...
            if (j.success && j.data.suggestions.length > 0) {
                ...
                for (var si = 0; si < j.data.suggestions.length; si++) {
                    ...
                    if (s.avatar) {
                        ...
                        apiHtml += '<span class="w-6 h-6 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">' + (displayTitle.charAt(0) || '?') + '</span>';
                    }
                ...
                // 底部"查看全部结果"链接
                apiHtml += '<a href="/search?q=' + encodeURIComponent(val) + '" class="s-item block px-4 py-2.5 text-center text-xs text-primary-500 hover:bg-gray-50 font-medium border-t border-gray-100 no-underline" onmousedown="event.preventDefault();hideSuggestions();document.getElementById(\'search-input\').value=\'' + val.replace(/'/g, "\\'") + '\';doSearch(event)">\u67e5\u770b\u5168\u90e8 ' + j.data.suggestions.length + ' \u6761\u7ed3\u679c \u2192</a>';
...
var labels = {products:'商品',kb_articles:'帮助中心',marketplace_apps:'应用市场',forum_posts:'社区',blog_posts:'博客',oa_articles:'互物号',users:'用户',official_accounts:'互物号账号'};
...
function renderPagination() {
    ...
    html += '<button onclick="goPage(' + (currentPage - 1) + ')" class="px-3 py-1.5 rounded-lg text-sm border border-gray-200 ' + (currentPage <= 1 ? 'bg-gray-50 text-gray-300 cursor-not-allowed' : 'bg-white text-gray-600 hover:bg-gray-50 cursor-pointer') + '" ' + (currentPage <= 1 ? 'disabled' : '') + '>上一页</button>';
    // Calculate visible page numbers
    var start = Math.max(1, currentPage - 2);
    var end = Math.min(totalPages, currentPage + 2);
    if (start > 1) html += '<span class="text-gray-300 text-sm">...</span>';
    for (var p = start; p <= end; p++) {
        html += '<button onclick="goPage(' + p + ')" class="px-3 py-1.5 rounded-lg text-sm border ' + (p === currentPage ? 'bg-primary-500 text-white border-primary-500' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50') + ' cursor-pointer">' + p + '</button>';
    }
    if (end < totalPages) html += '<span class="text-gray-300 text-sm">...</span>';
    html += '<button onclick="goPage(' + (currentPage + 1) + ')" class="px-3 py-1.5 rounded-lg text-sm border border-gray-200 ' + (currentPage >= totalPages ? 'bg-gray-50 text-gray-300 cursor-not-allowed' : 'bg-white text-gray-600 hover:bg-gray-50 cursor-pointer') + '" ' + (currentPage >= totalPages ? 'disabled' : '') + '>下一页</button>';
    html += '</div>';
    lm.innerHTML = html;
}
// ─── 相关搜索 ───
function renderRelatedSearches(q) {
    ...
    .then(function(d) {
        ...
        var html = '<div class="mt-6"><h3 class="text-sm font-medium text-gray-700 mb-3">相关搜索</h3><div class="grid grid-cols-2 gap-2">';
        for (var ki = 0; ki < keywords.length; ki++) {
            html += '<a href="/search?q=' + encodeURIComponent(keywords[ki]) + '" class="px-3 py-2 rounded-lg bg-gray-50 hover:bg-gray-100 text-sm text-gray-600 hover:text-primary-600 no-underline transition truncate">' + keywords[ki] + '</a>';
        }
        html += '</div></div>';
...
function followAccount(accountId, btnEl) {
    ...
    var isFollowed = btnEl.textContent === '已关注';
        ...
        if (res.success) {
            ...
            if (spans.length >= 2) {
                if (isFollowed) {
                    ...
                    spans[1].textContent = '+ 关注';
                    spans[1].className = 'inline-block px-3 py-1 rounded-full text-xs font-medium text-white bg-primary-500 hover:bg-primary-600 cursor-pointer';
                } else {
            ...
            btnEl.textContent = isFollowed ? '已关注' : '+ 关注';
...
async function execSearch(q) {
    ...
    try {
        ...
        if (rankedData.length === 0) {
            rl.innerHTML = '<div class="text-center py-16"><svg class="w-16 h-16 text-gray-200 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg><p class="text-gray-400 text-lg mb-1">未找到"'+q+'"的相关结果</p><p class="text-gray-300 text-sm mb-6">试试其他关键词</p></div><div id="trending-wrap"></div>';
            showNoResultTips(rl);
            // 异步加载热门推荐
            fetch('/api/meilisearch/trending').then(function(tr) { return tr.json(); }).then(function(tj) {
                if (tj.success && tj.data.length > 0) {
                    ...
                    var th = '<div class="max-w-4xl mx-auto mt-4"><div class="flex items-center gap-2 mb-4"><span class="text-base">🔥</span><h3 class="text-base font-semibold text-gray-700">热门推荐</h3></div><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">';
                    for (var ti = 0; ti < tj.data.length; ti++) {
                        ...
                        if (t.image) {
                            ...
                            th += '<div class="w-12 h-12 rounded-lg bg-gradient-to-br from-primary-100 to-primary-50 flex items-center justify-center text-lg flex-shrink-0">' + (t.icon || '📄') + '</div>';
                        }
                        th += '<div class="flex-1 min-w-0"><div class="text-sm font-medium text-gray-800 truncate group-hover:text-primary-600 transition">' + (t.title || '无标题') + '</div>';
...
function renderTypeFilters(data) {
    ...
    for (var t in stats) {
        ...
        if (cnt === 0) continue;
        html += '<button class="px-3 py-1.5 rounded-full text-sm font-medium transition ' + (activeTypeFilter===t?'bg-primary-500 text-white':'bg-gray-100 text-gray-600 hover:bg-gray-200') + '" onclick="setTypeFilter(\''+t+'\')">'+ (labels[t]||t) + ' ('+cnt+')</button>';
    }
    tf.innerHTML = html;
    tf.classList.remove('hidden');
    // 统计 + 排序切换
    var isAuth = !!localStorage.getItem('auth_token');
    var sortLabels = { relevance:'相关度', newest:'最新', smart:'🤖 智能推荐', ai:'🧠 AI 推荐', cf:'🔗 协同过滤', sequence:'📈 序列预测' };
    var statsHtml = '<div class="flex items-center justify-between flex-wrap gap-2 mb-3"><div class="text-sm text-gray-500">找到约 <strong>'+rankedData.length+'</strong> 条结果（共 '+Object.keys(stats).length+' 个分类）</div>';
    statsHtml += '<div class="flex items-center gap-2">';
    statsHtml += '<label class="text-xs text-gray-400 hidden sm:inline">排序：</label>';
    statsHtml += '<select id="sort-select" onchange="setSort(this.value)" class="text-sm border border-gray-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-300 cursor-pointer">';
    var sortOptions = [['relevance','相关度'],['newest','最新']];
    if (isAuth) sortOptions.push(['smart','🤖 智能推荐'],['ai','🧠 AI 推荐'],['cf','🔗 协同过滤'],['sequence','📈 序列预测']);
...
function renderResults() {
    ...
    for (var i = 0; i < pageItems.length; i++) {
        ...
        if (type === 'products') {
            ...
            var priceStr = price !== null ? (price > 0 ? '¥' + price.toFixed(2) : '免费') : '';
            ...
            html += '<span class="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-50 rounded text-xs text-gray-500">商品</span>';
            ...
            if (sales > 0) html += '<span class="text-xs text-gray-400">已售 '+sales+'</span>';
        ...
        if (type === 'official_accounts') {
            ...
            // 头像（靠左）
            html += '<a href="'+link+'" class="w-14 h-14 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white font-bold text-xl overflow-hidden flex-shrink-0 relative">';
            ...
            html += '<span class="relative">'+((h.name||'号').charAt(0))+'</span></a>';
            // 中间信息区
            html += '<div class="flex-1 min-w-0">';
            ...
            if (vi && vi.name) {
                var vType = vi.type === 'enterprise' ? '企业认证' : '个人认证';
            ...
            html += '</div>';
            html += '<div class="text-sm text-gray-500 mt-0.5 truncate">'+(desc||'暂无简介')+'</div>';
            ...
            if (isAuth) {
                html += '<span class="inline-block px-3 py-1 rounded-full text-xs font-medium border border-gray-200 bg-gray-100 text-gray-500 cursor-default" style="display:none">已关注</span>';
                html += '<span class="inline-block px-3 py-1 rounded-full text-xs font-medium text-white bg-primary-500 hover:bg-primary-600 cursor-pointer" onclick="followAccount(\''+h.id+'\',this)">+ 关注</span>';
            } else {
                html += '<a href="/build/login" class="inline-block px-3 py-1 rounded-full text-xs font-medium text-white bg-primary-500 hover:bg-primary-600 no-underline">+ 关注</a>';
            }
            html += '</div></div></div>';
            continue;
        }
        ...
        if (type === 'forum_posts') {
            ...
            if (pa) {
                ...
                html += '<span class="inline-block w-8 h-8 rounded-full bg-gradient-to-br from-primary-300 to-primary-500 text-center leading-8 text-white text-sm font-bold align-middle">'+(pu.charAt(0)||'?')+'</span>';
            }
        ...
        if (type === 'users') {
            ...
            if (ua) {
                ...
                html += '<span class="w-10 h-10 rounded-full bg-gradient-to-br from-primary-300 to-primary-500 flex items-center justify-center text-white text-sm font-bold">'+(un.charAt(0)||'?')+'</span>';
            }
        ...
        if (type === 'kb_articles') {
            ...
            html += '<span class="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-50 rounded text-gray-500">帮助中心</span>';
        ...
        if (type === 'blog_posts') {
            ...
            if (bt) {
                ...
                var diff = Math.floor((now - created) / 1000);
                if (diff < 60) bt = '刚刚';
                else if (diff < 3600) bt = Math.floor(diff/60) + '分钟前';
                else if (diff < 86400) bt = Math.floor(diff/3600) + '小时前';
                else if (diff < 2592000) bt = Math.floor(diff/86400) + '天前';
        ...
        if (authorName) {
            ...
            if (avatar) {
                ...
                html += '<span class="w-5 h-5 rounded-full bg-gradient-to-br from-primary-300 to-primary-500 flex items-center justify-center text-white text-[10px] font-bold">'+(authorName.charAt(0)||'?')+'</span>';
            }
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    else if(q){document.getElementById('search-input').value=q;execSearch(q).then(function(){if(type&&labels[type]){setTypeFilter(type);}});}else if(type&&labels[type]){document.getElementById('search-input').placeholder='输入关键词搜索 '+(labels[type]||type)+'...';}
});
</script>
