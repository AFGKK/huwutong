...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; padding: 40px; background: #f5f5f5;">
    <div style="max-width: 600px; margin: 0 auto; background: #fff; border-radius: 8px; padding: 32px;">
        <h2 style="margin-top: 0; color: #409eff;">{$this->title}</h2>
        <p style="color: #666; line-height: 1.6;">{$greeting}</p>
        <p style="color: #333; line-height: 1.6;">{$this->content}</p>
HTML
            . ($actionUrl ? <<<HTML
        <p style="margin: 24px 0; text-align: center;">
            <a href="{$actionUrl}" style="display: inline-block; padding: 12px 24px; background: #409eff; color: #fff; text-decoration: none; border-radius: 4px; font-size: 15px;">{$actionText}</a>
        </p>
HTML
            : '')
            . <<<HTML
